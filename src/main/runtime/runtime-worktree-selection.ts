...
export function getRuntimeWorktreeRemovalOptionsKey(
  options: Pick<
    ...
    'force' | 'runHooks' | 'allowUnverifiedPtyStop' | 'allowFailedArchiveHook' | 'keepBranch'
  >
): string {
  ...
  const branchKey = options.keepBranch ? 'keep-branch' : 'delete-branch'
  return `${options.force ? 'force' : 'normal'}:${hooksKey}:${ptyKey}:${archiveKey}:${branchKey}`
}
