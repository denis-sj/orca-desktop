...
export function getWorktreeRemovalOptionsKey(
  args: Pick<
    RemoveWorktreeArgs,
    'force' | 'allowUnverifiedPtyStop' | 'skipArchive' | 'allowFailedArchiveHook' | 'keepBranch'
  >
): string {
  ...
  const branchKey = args.keepBranch === true ? 'keep-branch' : 'delete-branch'
  return `${forceKey}:${archiveKey}:${ptyKey}:${archiveFailureKey}:${branchKey}`
}
